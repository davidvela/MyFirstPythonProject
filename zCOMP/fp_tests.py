...
def tests_json(excel):
    ...
    if excel == True: 
        writer = pd.ExcelWriter(LOGDAT+'pandas.xlsx', engine='xlsxwriter')
...
def main():
    ...
    ex = executions[1]
    LAB_DS     = LOGDAT + ex['path'] + "/datal.csv"
    COL_DS     = LOGDAT + ex['path'] + "/datac.csv" 
    ALL_DSJ    = LOGDAT + ex['path'] + "data_json1.txt"
    ALL_DS     = LOGDAT + ex['path'] + "datasc.csv"
   
    num = ex['n'] 
    if num == 1: dc = tests_json(ex['excel'])
    if num == 2: 
        #filters = [ ["", 0], ['>', 60], ['<', 93]]
        filters = [ ["", 0]]
        for i in range(len(filters)):
            dc = tests_classifN_100(filters[i])
    if num == 3: dc, dt, de = tests_classif(ex['filter'][0] )
    return
